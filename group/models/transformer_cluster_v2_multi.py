...
from group.models.routing_transformer_v2 import RoutingTransformer
...
class TransformerEncoderLayer_cluster(nn.Module):
    ...
    def __init__(self, d_model, nhead,total_size,window_size ,num_clusters=None,dim_feedforward=2048, dropout=0.1,
        ...
        self.k_attn_1=RoutingTransformer(dim=d_model,max_seq_len=total_size,heads=nhead,window_size=6,num_clusters=num_clusters,
                                       causal=False,attn_dropout=0.1,shared_qk=True)
        self.k_attn_2 = RoutingTransformer(dim=d_model, max_seq_len=total_size, heads=nhead, window_size=4,
                                           num_clusters=num_clusters,
                                           causal=False, attn_dropout=0.1, shared_qk=True)
        self.k_attn_3 = RoutingTransformer(dim=d_model, max_seq_len=total_size, heads=nhead, window_size=2,
                                           num_clusters=num_clusters,
                                           causal=False, attn_dropout=0.1, shared_qk=True)
    ...
    def forward_post(self,
        ...
        #src2,loss2=self.k_attn_1(q)
        src3,loss3=self.k_attn_2(q)
        src4,loss4=self.k_attn_3(q)
        src=src+src3+src4
    ...
    def forward_pre(self, src,
        ...
        #                      key_padding_mask=src_key_padding_mask)[0]
        #src2, loss = self.k_attn_1(q)
        src3, loss3 = self.k_attn_2(q)
        src4, loss4 = self.k_attn_3(q)

        src = src +self.dropout1(src3)+self.dropout1(src4)
        src2 = self.norm2(src)
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src2))))
        src = src + self.dropout2(src2)
        return src,loss4+loss3
