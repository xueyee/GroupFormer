...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False):
        super().__init__()
        self.self_attn =MultiheadAttention(d_model, nhead, dropout=dropout,attn_mask_manner='knn')
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            return multi_head_attention_forward(
                ...
                v_proj_weight=self.v_proj_weight,attn_mask_manner=self.attn_mask_manner)
        else:
            return multi_head_attention_forward(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                ...
                training=self.training,
                key_padding_mask=key_padding_mask, need_weights=need_weights,
                attn_mask=attn_mask,attn_mask_manner=self.attn_mask_manner)
