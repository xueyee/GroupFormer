...
class STTR_block(nn.Module):
    def __init__(self,config,embed_feat):
        ...
        self.embed_features=embed_feat
        #self.k=config.k
        encoder_layer_st = TransformerEncoderLayer_cluster(self.embed_features, config.Nhead,total_size=config.total_size,window_size=config.window_size ,num_clusters=config.num_clusters,dropout=config.dropout_porb,
                                                      normalize_before=True)
...
class cluster_nttr_v2(nn.Module):
    def __init__(self,config):
        super(cluster_nttr_v2, self).__init__()
        ...
        for i in range(self.num_STTR_layers):
            config.window_size=self.window[i]
