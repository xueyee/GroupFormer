...
def knn_attn_mask(x, y=None, k=10):
    """
    ...
    idx_base=torch.arange(0,B*head).view(-1,1,1)*M
    idx=idx_knn+idx_base
    idx=idx.view()
    attn_mask=torch.zeros_like(pairwise_distance).view(-1)
    attn_mask[idx]=1
...
class ST_plus_TR_knn(nn.Module):
    def __init__(self,config):
        super(ST_plus_TR_knn, self).__init__()
