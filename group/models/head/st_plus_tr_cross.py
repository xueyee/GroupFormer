...
class ST_plus_TR_cross(nn.Module):
    def __init__(self,config):
        super(ST_plus_TR_cross, self).__init__()
        ...
        self.STTR_module.append(ST_plus_TR_block_actor(config,embed_feat))
        self.STTR_module.append(ST_plus_TR_block_temp(config,embed_feat))
    ...
    def forward(self,x,global_token):
        ...
        tgt=token
        #print(tgt.shape)
        memory,tgt=self.STTR_module[0](memory,tgt)
        memory,tgt=self.STTR_module[1](memory,tgt)
